...
SELECT * FROM EMPLOYEE
WHERE FIRSTNAME LIKE '_[AEOIU][AEOIU]%' AND CITY LIKE 'R%'
--Part – B:
...
SELECT * FROM EMPLOYEE
WHERE FIRSTNAME LIKE '%[AEIOU]' AND DEPARTMENT LIKE '[AEOIU]%' AND SALARY BETWEEN 10000 AND 15000
...
SELECT * FROM EMPLOYEE
WHERE FIRSTNAME LIKE '_[AEUIO]%' AND LASTNAME LIKE '%R' AND SALARY <12000
--29. Display employees whose CITY is NOT NULL and FIRSTNAME does not start with vowel and DEPARTMENT
--not in (‘HR’, ‘IT’).
SELECT * FROM EMPLOYEE
WHERE FIRSTNAME NOT LIKE '[AEUIO]%' AND CITY IS NOT NULL AND DEPARTMENT NOT IN ('HR','IT')
