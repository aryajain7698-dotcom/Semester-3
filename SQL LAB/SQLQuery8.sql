...
SELECT COUNT(SALARY) AS COUNT_EMPLOYEE
	   FROM EMPLOYEE
	   WHERE DEPARTMENT='HR'
...
SELECT CITY,COUNT(SALARY) AS COUNT_EMPLOYEE
	   FROM EMPLOYEE
	   GROUP BY CITY
--19. Count number of employees in each city where gender is MALE.
SELECT COUNT(EID) AS COUNT_EMPLOYEE
	   FROM EMPLOYEE
	   WHERE GENDER='MALE'
...
SELECT MAX(SALARY)  AS MAX_SALARY,DEPARTMENT,CITY
	   FROM EMPLOYEE
	   WHERE CITY LIKE 'R%'
